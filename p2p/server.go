...
func (srv *Server) Start() (err error) {
	...
	if srv.ListenAddr != "" {
		if err := srv.setupListening(); err != nil { // rlpx
			return err
		}
	}
	if err := srv.setupDiscovery(); err != nil { // 节点发现
		return err
	}
	srv.setupDialScheduler() // 连接管理

	srv.loopWG.Add(1)
	go srv.run() // 监听handshake peer的连接管理 Handshake
	return nil
}
...
func (srv *Server) setupDiscovery() error {
	...
	conn, err := net.ListenUDP("udp", addr) //  这里真正开启监听
	if err != nil {
		return err
	}
	...
	srv.localnode.SetFallbackUDP(realaddr.Port) // 获得5个nursey节点；//设置最开始的启动节点。当找不到其他的节点的时候。 那么就连接这些启动节点。这些节点的信息是写死在配置文件里面的。
...
func (srv *Server) setupDialScheduler() {
	...
	if srv.ntab != nil {
		config.resolver = srv.ntab // dth: 这里的ntab是一个discover.Resolver
	}
	if config.dialer == nil { // 使用了TCLPDialer
...
func (srv *Server) listenLoop() {
	...
	for {
		...
		go func() {
			srv.SetupConn(fd, inboundConn, nil) // SetupConn,这个函数执行握手协议，并尝试把连接创建位一个peer对象。
...
func (srv *Server) SetupConn(fd net.Conn, flags connFlag, dialDest *enode.Node) error {
	...
	if dialDest == nil {
		c.transport = srv.newTransport(fd, nil) // 被动连接
	} else {
		c.transport = srv.newTransport(fd, dialDest.Pubkey()) // 主动向外连 主动连要加公钥
	}
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	// Run the RLPx handshake. // 运行安全通道的协商过程
